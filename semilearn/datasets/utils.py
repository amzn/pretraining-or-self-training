...
def split_ssl_data(args, data, targets, num_classes,
                   ...
                   lb_imbalance_ratio=1.0, ulb_imbalance_ratio=None,
...
def sample_labeled_unlabeled_data(args, data, target, num_classes,
    ...
    '''
    if not ulb_imbalance_ratio:
        dump_dir = os.path.join(base_dir, 'data', args.dataset, 'labeled_idx')
    else:
        dump_dir = os.path.join(base_dir, 'data', args.dataset, 'unbalanced_labeled_idx')
    ...
    if not ulb_imbalance_ratio:
        ...
        ulb_samples_per_class_tmp = [int(ulb_num_labels / num_classes)] * num_classes
        ulb_samples_per_class = [num * ratio for num, ratio in zip(ulb_samples_per_class_tmp, ulb_imbalance_ratio)]
        # imbalanced setting
